...
export default {
  ...
  methods: {
    ...
    searchOnTable() {
      this.impressoras = searchByName(this.impressoras, this.search);
    },
    aguardandoChegar() {
         axios.get("http://localhost:5000/impressoras/1").then((response) => {
      this.impressoras = response.data;
    });
        this.nomeEstagio = "Aguardando Chegar na Zhaz";
    },
    aguardandoVistoria() {
         axios.get("http://localhost:5000/impressoras/3").then((response) => {
      this.impressoras = response.data;
    });
        this.nomeEstagio = "Aguardando Vistoria";
    },
    aguardandoAprovacao() {
         axios.get("http://localhost:5000/impressoras/6").then((response) => {
      this.impressoras = response.data;
    });
        this.nomeEstagio = "Aguardando Aprovação";
    },
    Aprovado() {
         axios.get("http://localhost:5000/impressoras/8").then((response) => {
      this.impressoras = response.data;
    });
        this.nomeEstagio = "Aprovado !!!";
    },
    emManutencao() {
         axios.get("http://localhost:5000/impressoras/9").then((response) => {
      this.impressoras = response.data;
    });
        this.nomeEstagio = "Em Manutenção";
    },
    manutencaoConcluida() {
         axios.get("http://localhost:5000/impressoras/11").then((response) => {
      this.impressoras = response.data;
    });
        this.nomeEstagio = "Manutenção Concluida";
    },
    reprovado() {
         axios.get("http://localhost:5000/impressoras/17").then((response) => {
      this.impressoras = response.data;
    });
        this.nomeEstagio = "Reprovado :(";
    },
    finalizado() {
         axios.get("http://localhost:5000/impressoras/15").then((response) => {
      this.impressoras = response.data;
    });
